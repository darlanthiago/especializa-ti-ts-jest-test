...
describe("Cart", () => {
  ...
  it("should has two items in cart and add some product one", () => {
    const { sut } = makeSut();
    const product_one = makeProduct(uuidv4(), "Product One", 100);
    sut.addProductToCart(product_one);
    const product_two = makeProduct(uuidv4(), "Product Two", 278);
    ...
    expect(sut.items[0].product).toBe(product_one);
    expect(sut.items[1].product).toBe(product_two);
  });
  ...
  it("should have one if add two and remove one", () => {
    ...
    expect(sut.items[0].quantity).toBe(4);
  });
